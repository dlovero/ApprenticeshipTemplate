require 'rails_helper'

RSpec.describe SaleController, type: :controller do
  before do
    User.create!(user_name: 'tester', password: '1234567')
    a_book=Book.create!(title: 'El hombre de la mascara de hierro', isbn: '1234567890', price: 25)
    another_book=Book.create!(title: 'El gato negro', isbn: '1231231231', price: 20)
    a_credit_card=CreditCard.create!(credit_card_owner: 'Pepe Grillo', credit_card_number: '1234567890123456', expiration_date: Date.new(3030, 12, 1), user_id: 1)
    a_cart=Cart.create!(user_id: 1)
    a_cart.add(a_book, 10)
    a_cart.add(another_book, 11)
    Cashier.new(MerchantProcessor.new).checkout(a_cart, a_credit_card)
  end

  context 'When you want to list the purchases of an user' do
    context 'and you can' do
      it 'should return a json with a list of all the purchases' do
        post :list, userId: 1, password: '1234567'
        expect(response).to have_http_status :ok
        expect(JSON.parse(response.body)).to eq({"1" => [{"1234567890" => 10}, {"1231231231" => 11}]})
      end
    end

    context 'and you cant' do
      it 'should return an error' do
        post :list, userId:2, password:'1234567'
        expect(response).to have_http_status :not_found
        expect(JSON.parse(response.body)).to eq({"error"=>"Could not list"})
      end
    end
  end

end
